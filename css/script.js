...
$('.welcome-slide__button').on('click', function() {
  ...
  function toggleAnimation() {
    ...
    var divMobCont2 = $('#appointment-button--mob-menu');
    if (divMobCont2.is(e.target)) || (!divMobCont.is(e.target) && divMobCont.has(e.target).length === 0) { // и не по его дочерним элементам
      mobileMenuAnimation.reverse();
    mobileMenuToggle = true;
}
});
