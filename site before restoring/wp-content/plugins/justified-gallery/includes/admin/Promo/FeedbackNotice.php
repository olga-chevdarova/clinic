		...
		if ($this->allow_display())
		{
			...
			<div class="dgwt-jg-review-notice">
				<div class="dgwt-jg-review-notice-logo"></div>
	                <?php printf( __( "Hey %s, it's Damian Góra from %s. You have used this free plugin for some time now, and I hope you like it!", DGWT_JG_DOMAIN ),
		                '<strong>' . $current_user->display_name . '</strong>',
		                '<strong>' . DGWT_JG_NAME . '</strong>'
	                ); ?>
                <br />
	                <?php printf(__("I have spent countless hours developing it, and it would mean a lot to me if you %ssupport it with a quick review on WordPress.org.%s", DGWT_JG_DOMAIN),
				...
				<div class="button-container">
					<a href="<?php echo self::REVIEW_URL; ?>" target="_blank" data-link="follow" class="button-secondary dgwt-review-notice-dismiss">
						...
						<?php _e("Review Justified Gallery", DGWT_JG_DOMAIN); ?>
					</a>
					<a href="#" class="button-secondary dgwt-review-notice-dismiss">
						<span class="dashicons dashicons-no-alt"></span>
						<?php _e("No thanks", DGWT_JG_DOMAIN); ?>
					</a>
				</div>
			</div>
			<?php
